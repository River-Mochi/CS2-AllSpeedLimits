            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(SpeedLimitsSetting.SpeedUnitPreference)),
                    ...
                    "<AUTO> theo loại bản đồ:\n" +
                    "- EU = KM/H, NA = MPH.\n" +
                    "Chọn <KM/H hoặc MPH> để cố định kiểu hiển thị đó."
                },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(SpeedLimitsSetting.SyncSliderWithSelection)),
                    "**[ ✓ ] Nên bật**\n" +
                    "Bật: bấm một đoạn sẽ đưa thanh trượt đến tốc độ hiện tại của đoạn đó thay vì bắt đầu từ 5.\n" +
                    "- Nếu chọn nhiều đoạn, đoạn đầu tiên vẫn đặt vị trí bắt đầu của thanh trượt.\n" +
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(SpeedLimitsSetting.PanelSliderIncrement)),
                    "Đặt bước nhảy của thanh trượt trong bảng giới hạn tốc độ thành phố.\n" +
                    "<Mặc định = 10>"
                },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(SpeedLimitsSetting.TooltipFontScale)),
                    "Mod này cho phép chữ lớn hơn trong ô trợ giúp khi trỏ chuột lên các tính năng giới hạn tốc độ.\n" +
                    "<Mặc định 110%>"
                },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(SpeedLimitsSetting.DoubleSpeedDisplay)),
                    ...
                    "Hữu ích nếu mod chú thích khác hoặc Scene Explorer hiện giá trị nội bộ nhân đôi của game và bạn muốn chúng khớp nhau.\n" +
                    ...
                    "Lưu ý: <ký hiệu trên đường là hình trang trí> nên có thể không khớp dữ liệu tốc độ thật của mẫu game. Biển 35 mph có thể là ~31 mph. Game định nghĩa tốc độ đường theo hệ mét trước rồi mới chuyển đổi."
                },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(SpeedLimitsSetting.ClearAllCustomSpeeds)),
                    ...
                    "<============>\n" +
                    "\n" +
                    "Nút này khôi phục mặc định game đã biết ở mọi nơi mod đã áp dụng tốc độ tùy chỉnh.\n" +
                    "Sau khi xong, hãy tạo **BẢN LƯU MỚI** trước khi gỡ mod.\n" +
                    "Nếu gỡ mod mà không dùng mục này, tốc độ tùy chỉnh sẽ còn lại cho đến khi bạn đổi đường bộ, đường ray hoặc đường thủy."
                },

                { m_Setting.GetOptionWarningLocaleID(nameof(SpeedLimitsSetting.ClearAllCustomSpeeds)),
                    "Mục này khôi phục mọi giới hạn tốc độ tùy chỉnh do mod áp dụng về mặc định game đã biết.\n" +
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(SpeedLimitsSetting.UsageText)),
                    ...
                    "4. Các nút <Cài sẵn> áp dụng ngay.\n" +
                    "\n" +
                    "<Toàn thành phố>\n" +
